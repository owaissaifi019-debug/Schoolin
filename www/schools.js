function initSchoolsDirectory() {
  ...
  function renderSchools() {
    ...
    if (filtered.length === 0) {
      if (isMobile) {
        gridContainer.innerHTML = `
          <div style="grid-column: 1/-1; text-align: center; padding: 40px 20px; background: var(--primary-light); border-radius: var(--radius-md); border: 1px solid rgba(0, 102, 200, 0.12); margin-top: 10px;">
            <span style="font-size: 1.8rem; background: var(--white); width: 48px; height: 48px; display: inline-flex; align-items: center; justify-content: center; border-radius: var(--radius-sm); box-shadow: var(--shadow-sm); margin-bottom: 12px;">🏫</span>
            <h4 style="font-weight: 700; color: var(--primary); font-size: 1.1rem; margin-bottom: 4px;">No schools found</h4>
            <p style="font-size: 0.9rem; color: var(--text-muted); margin-bottom: 16px;">Can't find your school?</p>
            <button class="btn btn-primary" id="btn-suggest-not-found" style="padding: 10px 20px; font-size: 0.88rem;">Suggest a School</button>
          </div>
        `;
      } else {
        gridContainer.innerHTML = `
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initSchoolsDirectory);
} else {
