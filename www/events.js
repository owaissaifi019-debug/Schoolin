function initEvents() {
  ...
  function renderEvents() {
    ...
    // Toggle filters-active class to show/hide Clear button & count label on mobile
    const filterBar = document.querySelector('.filter-bar');
    if (filterBar) {
      const isFiltered = searchQuery.trim() !== '' || activeCategory !== 'all';
      filterBar.classList.toggle('filters-active', isFiltered);
    }
    ...
    filtered.forEach(item => {
      ...
      card.innerHTML = `
        ...
        <div class="event-card-grid-content">
          ...
          <div class="event-card-footer-row">
            <div class="event-card-regs-info">
              <span class="event-card-regs-number">${String(item.registrations || '0').split(' ')[0]}</span>
              <span class="event-card-regs-label">Registrations</span>
            </div>
            <button class="btn btn-primary btn-register-action" data-title="${item.title}">Register</button>
          </div>
        </div>
      `;
      
      // Navigate to event detail page on card tap (mobile) or click
      card.addEventListener('click', (e) => {
        // Do not trigger navigation if clicking a button (register)
        if (e.target.closest('button')) {
          return;
        }
        window.location.href = `event-detail.html?id=${item.id}`;
      });
